...
$size = isset($_CFG['page_size'])  && intval($_CFG['page_size']) > 0 ? 4 : 4;
...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update','sales_volume','comment_num'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $smarty->assign('categories',       get_child_tree($cat_id)); // 分类树
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id,(SELECT count(c.comment_id) FROM '. $GLOBALS['ecs']->table('comment') 
	.' as c where c.id_value = g.goods_id)as comment_num,'
	.' (SELECT SUM(c.comment_rank)/comment_num from '. $GLOBALS['ecs']->table('comment') .' as c where c.id_value = g.goods_id) as comment_rank, '
	.' g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
