...
function hiddendiv(){
 ...
 function getchildid(){
	 ...
	 var url =  value.split('_')[1];
	 window.location.href=url;
	// Bingcategroychild(cat_id);
 }
 function getchildid2(){
	 var url= $('#categroychild').val();
	 window.location.href=url;
 }
 
function Bingcategroychild(id){
	$('#categroychild').empty();
	var url="../get.php?action=Bingcategroychild";
	$.ajax({
					type: "POST",
					url: url,
					dataType: "json",
					...
					success: function(json){
						if(json&&json.length>0){
							document.getElementById("categroychild").style.display = "";
							$.each(json,function(i,item){
								if(i==0){
									document.getElementById("categroyname").innerText=item.name;
								}
								$("<option value='"+item.url+"'>"+item.name+"</option>").appendTo($('#categroychild'));
							});
						}
						else {
							document.getElementById("categroychild").style.display = "none";
						}
					}
	});
 }
