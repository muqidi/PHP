...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$smarty->assign('record_count',       getrecord_count($goods_id));
		$smarty->assign('bought_notes',       getbought_notes($goods_id));
		...
		//$smarty->assign('goods_sales',          get_goods_sales($goods_id));                       // 商品的销售排名
        //获取tag
...
function getrecord_count($id){
	 $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' AND comment_type = 0 AND status = 1 AND parent_id = 0");
		   return $count;
		
}

   function getbought_notes($id){
	   $sql = 'SELECT count(*) ' .
