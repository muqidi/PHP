 ...
 * @return  array
 */
function get_categories_childtree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
...
function get_index_categories_type_good(){
	 $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = 0 AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
	$res = $GLOBALS['db']->getAll($sql);
...
//获取指定分类下面的促销、热卖、打折、推荐商品
function get_index_goods( $cat_id=0,$type="",$num = 4){
...
//获取首页需要的各种商品，4个。
function get_index_goods1($cat_id=0,$type=""){
	    $sql = 'SELECT g.goods_id, g.goods_name,g.goods_thumb, g.market_price, g.shop_price , g.promote_price,  '
                .'promote_start_date, promote_end_date,g.goods_img  '
                .'FROM  '. $GLOBALS['ecs']->table('goods') . 
				"AS g LEFT JOIN  ". $GLOBALS['ecs']->table('category') . 
				" as c ON c.cat_id = g.cat_id WHERE c.parent_id = '$cat_id' and ";
		if($type =="hot"){
			$sql .= ' g.is_hot =1';
		}else if($type=="new"){
			$sql .= ' g.is_new =1';
		...
		$sql .=" and  g.is_on_sale = 1 AND  g.is_alone_sale = 1 AND g.is_delete = 0  ORDER BY g.sort_order, g.last_update  DESC LIMIT 4 ";
		...
		foreach ($result AS $idx => $row)
        {
			...
			$goods[$idx]['shop_price']  =price_format($row['shop_price']) ;
			$goods[$idx]['market_price']  = price_format($row['market_price']);
			$goods[$idx]['goods_url']  = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
			//$goods[$idx]['thumb']  = get_image_path($row['goods_id'], $row['goods_thumb'], true);
			$goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
			if ($row['promote_price'] > 0)
            {
			...
			//$goods[$idx]['goods_short_name']=$GLOBALS['_CFG']['goods_name_length'] > 0 ?sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
		}
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.cat_id,getCateGoryParentId(g.cat_id) as cat_parent_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['id']           = $row['goods_id'];
			$goods[$idx]['cat_parent_id']           = $row['cat_parent_id'];
            $goods[$idx]['name']         = $row['goods_name'];
            $goods[$idx]['brief']        = $row['goods_brief'];
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id,getCateGoryParentId(g.cat_id) as cat_parent_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
