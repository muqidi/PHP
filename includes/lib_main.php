...
function update_user_info()
{
    ...
    /* 查询会员信息 */
    $time = date('Y-m-d');
    $sql = 'SELECT u.address_id, u.user_money,u.email, u.pay_points, u.user_rank, u.rank_points, '.
...
$size =2;
    ...
    $pager['page_count']   = $page_count;
    $pager['page_prev']    = $page > 1 ? "javascript:gotoPage(" .($page-1). ",$id,$type)" : '';
	...
	if($page_count<6)
	{
		...
		while($dx<$page_count){
		$page_n[$dx]['key']=$dx;
		$page_n[$dx]['url']='javascript:gotoPage(' .($dx) . ",$id,$type)";
		$dx++;
		}
	}
	else if ($page < 5){
		...
		$page_n[$dx]['key']='...';
		$page_n[$dx]['url']='#';
	}else if($page>=$page_count - 4){

		$page_n[0]['key']='...';
		$page_n[0]['url']='#';
		$dx=3;
		while($dx>0){
		$page_n[$dx]['key']=$page_count-$dx;
		$page_n{$dx}['url']='javascript:gotoPage(' .($page_count-$dx) . ",$id,$type)";
		$dx--;
		}
	}else{
		$page_n[0]['key']='...';
		...
		$dx=-2;
		while($dx<3){
		$page_n[$dx+3]['key']=$page+$dx;
		$page_n[$dx+3]['url']='javascript:gotoPage(' .($page+$dx) . ",$id,$type)";
		$dx++;
		}
		$page_n[$dx+3]['key']='...';
		$page_n[$dx+3]['url']='#';
	}
	$pager['page_number'] =$page_n;

	
	
    $pager['page_next']    = $page < $page_count ? 'javascript:gotoPage(' .($page + 1) . ",$id,$type)" : '';
    $pager['page_last']    = $page_count > 1 ? 'javascript:gotoPage(' .$page_count. ",$id,$type)"  : '';
