...
$(function(){
	...
	$("#pass").focus(function(){
		...
		document.getElementById('error_tip').innerText="请输入8-20位，建议数字，字母，符号混合的密码。";
	...
	$("#pass").blur(function(){
		...
		if(pass==""){
			...
			document.getElementById('error_tip').innerText="密码长度少于8位！";
		}
		else document.getElementById('error_tip').innerText="";
		});
	
	$("#cpass").blur(function(){
		var cpass = $('#cpass').val();
		if(cpass==""){
			document.getElementById('error_tip').innerText="确认密码不能为空！";
		}else{
			if(cpass!= $('#pass').val()){
				document.getElementById('error_tip').innerText="两次输入的密码不一致！";
				return false;
			}
			document.getElementById('error_tip').innerText="";
		}
		});
});
